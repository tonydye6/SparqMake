...
describe("buildImagePrompt — designer persona injection & precedence", () => {
  it("includes the persona fingerprint block when a persona is set", () => {
    ...
    expect(prompt).toContain('STYLE INSPIRATION — inspired by "Neo-Brutalist Editorial"');
  ...
  it("omits the persona block when no persona is set", () => {
    ...
    expect(prompt).not.toContain("STYLE INSPIRATION");
  ...
  it("states persona precedence over the design style when both are active", () => {
    ...
    expect(prompt).toContain("PRECEDENCE: Where this style inspiration conflicts with the DESIGN STYLE above, this style inspiration takes precedence for look and feel.");
    // Persona block must come after the design style block so "above" reads correctly.
    expect(prompt.indexOf("STYLE INSPIRATION")).toBeGreaterThan(prompt.indexOf("DESIGN STYLE"));
  });

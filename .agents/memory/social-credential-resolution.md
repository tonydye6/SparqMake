...
**Reconstruction note (2026-07-02):** The original resolver file was lost from the repo despite its commit claiming to add it (consumers imported a nonexistent module — TS2307 in four files; server could not boot, typecheck failed). It was rebuilt from consumer call sites and the env-name inventory above. Lessons: when a commit message claims a new file was added, verify it exists in the tree (`git cat-file -e HEAD:<path>`) before trusting a "green baseline" memory; if a merge lands and typecheck suddenly reports a missing module, suspect a dropped new file before suspecting your own changes.
